...
def index_docs():
    """
    Reads PDFs from backend/data, chunks them, and upload vectors to Azure AI Search.
    """
    # Define Paths
    ...
    try:
        ...
        logger.error("Please verify your Azure Search endpoint, API key, and index_name.")
        return 

    # Find PDF Files
    pdf_files = glob.glob(os.path.join(data_dir, "*.pdf"))
    if not pdf_files:
        logger.warning(f"No PDFs found in {data_dir}. Please add files.")
        return 
    ...
    if all_splits:
        ...
        try: 
            ...
            logger.info("="*60)
            logger.info("Indexing Completed! The Knowledge Base is ready.")
